...
keypoints = np.load("/Users/esthrelar/Documents/Programming/23FW_UnivProject/joint_angles_calculate/euler.npy")
...
#    armature.pose.bones[bone_name].rotation_mode = 'YXZ'
#    
...
#        if i == 0:
#            armature.pose.bones[bone_name].rotation_euler.z = angle
#        elif i == 1:
#            armature.pose.bones[bone_name].rotation_euler.x = angle
#        elif i == 2:
#            armature.pose.bones[bone_name].rotation_euler.y = angle
#        
#    if bone_name == 'mixamorig9:Hips':
#        armature.pose.bones[bone_name].rotation_euler.z += math.radians(0)
#        armature.pose.bones[bone_name].rotation_euler.y += math.radians(0)
#        armature.pose.bones[bone_name].rotation_euler.x += math.radians(0)

#    if bone_name == 'mixamorig9:RightShoulder':
#    #        armature.pose.bones[bone_name].rotation_euler.z += math.radians(270)
#            armature.pose.bones[bone_name].rotation_euler.y += math.radians(-0)
#    #        armature.pose.bones[bone_name].rotation_euler.x += math.radians(90)
    
for framenum in range(1,len(keypoints)+1): # 1 ~ 531
    for frame_keypoints, bone_name in zip(keypoints[framenum-1], targets):
        print(f"keypoints[i] = {frame_keypoints}, targets = {bone_name}")
        
        for i, angle in enumerate(frame_keypoints):
            # put each bone_angle array into ZXY angles of this character's rotation angle
            if bone_name == 'mixamorig9:Neck':
                if i == 0:
                    armature.pose.bones[bone_name].rotation_euler.z = 0
                elif i == 1:
                    armature.pose.bones[bone_name].rotation_euler.x = 0
                elif i == 2:
                    armature.pose.bones[bone_name].rotation_euler.y = 0
                    
            elif bone_name == 'mixamorig9:LeftShoulder' or bone_name == 'mixamorig9:RightShoulder' or bone_name == 'mixamorig9:LeftForeArm' or bone_name == 'mixamorig9:RightForeArm':
                if i == 0:
                    armature.pose.bones[bone_name].rotation_euler.z = (-1) * angle
                elif i == 1:
                    armature.pose.bones[bone_name].rotation_euler.x = (-1) * angle
                elif i == 2:
                    armature.pose.bones[bone_name].rotation_euler.y = (-1) * angle
        ...
        # insert keyframe
        armature.pose.bones[bone_name].keyframe_insert('rotation_euler',frame=framenum)
        
# ----------------------------------------------------------
# 특정 bone의 이름 지정
bone_name = "mixamorig9:RightShoulder"
...
if bone:
    # 로컬 좌표계에서의 오일러 각도 출력
    local_rotation_euler = bone.rotation_euler
    print("Local Euler Rotation (degrees):", [math.degrees(angle) for angle in local_rotation_euler])
else:
